...
sortDragDrop = function (local)
{
    ...
    $('.playlists-tracks > ul').sortable({
      ...
      update:function(event,ui){
        ...
        local.playlist.set('#'+$(this).attr('id'));
        queue.children('span.badge').text($('#queue_sortable').find('li').length);
        $(local.audio).trigger('playlistUpdate',[ $(this) ] );
        Logger.debug("sortable updated");
      }
  ...
  $('#playlist li').draggable({
    ...
    start:function(event,ui){
          // enableQueue();
          $('#queue_sortable').css('border','2px dashed #CB8B1E');
          ui.helper.addClass('playlist-item');
          ui.helper.attr('style','position:relative;width:415px;min-height:70px;height:70px;background-color:#000;color:white;border:none;border:1px solid white;');
