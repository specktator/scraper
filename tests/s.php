...
class scrape {
  ...
  function getfolders($target){
    ...
    if(count($this->folders[1])>1){ // if there is not another folder getfiles()
      flush();
      // if($this->debug) {echo "<h3>Folders</h3><br>";}
      // if($this->debug) {var_dump($this->folders[1]);}

      foreach ($this->folders[1] as $index => $foldername) {
        if ($index == '0' and strpos($foldername,'/') !== FALSE or empty($foldername)) {continue;}
        $this->foldername = $foldername;
        if($this->debug) {echo "<h3>Folder scanning: ".$target.'/'.urldecode($foldername)."</h3><br>";}
        
        $this->__start($target.'/'.$foldername);
        // $this-> getfolders($target);
        $this->getfiles();
      }
  ...
  function getfiles(){
    ...
    if(!empty($this->files[1][0])){
      if($this->debug) {echo "<h3>Get files()</h3><br>";}
...
$obj->__start('http://localhost/music/','audio only');
// $obj-> getolders();
?>
</body>
</html>